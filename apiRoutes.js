...
router.get('/dishes', (req, res) => {
    ...
    let filteredDishes = data
        .filter(dish => !category || dish.category.toLowerCase() === category.toLowerCase())
        .filter(dish => !price || dish.price < parseFloat(price))
        .filter(dish => !name || dish.name.toLowerCase().includes(name.toLowerCase()))
        .filter(dish => isVegetarian === undefined || dish.isVegetarian === (isVegetarian === 'true'));
